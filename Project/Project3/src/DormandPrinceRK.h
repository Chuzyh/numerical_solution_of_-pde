...
private:
    int n_steps;
    double Ti,rho_max=2.0,rho_min=0.5,rho=0.8,q=4.0;
    vector<double> init;
...
public:
    ...
    void solve()
    {
        ...
        auto now=init;double tick=Ti/n_steps;
        ...
        for(int i=1;i<=n_steps;i++)
        {
            ...
            now=now+tick*(RKweights[0]*y0+RKweights[1]*y1+RKweights[2]*y2+RKweights[3]*y3+RKweights[4]*y4+RKweights[5]*y5+RKweights[6]*y6);
